...
export function Expertise() {
  ...
  useEffect(() => {
    const id = setInterval(() => setIndex((prev) => (prev + 1) % experts.length), 10000);
    return () => clearInterval(id);
  }, [experts.length]);

  const goTo = (i: number) => setIndex((i + experts.length) % experts.length);
    ...
    <section className="mx-auto mt-24 w-full max-w-6xl px-6">
      ...
      <div className="relative mt-12" ref={containerRef}>
        <div className="overflow-hidden rounded-3xl border border-emerald-900/10 bg-white/95 shadow-brand">
          ...
          >
            {experts.map((person) => (
              <Slide key={person.name} expert={person} width={width} />
            ))}
          </motion.div>
        </div>
...
function Slide({ expert, width }: { expert: Expert; width: number }) {
  return (
    <article
      className="grid min-w-full gap-8 p-6 text-slate md:grid-cols-[0.9fr_1.1fr] md:items-center md:p-8"
      style={{ width: width || '100%' }}
    >
      <div className="relative overflow-hidden rounded-3xl border border-emerald-900/10 bg-white/95 p-6 shadow-brand">
        ...
        <p className="mt-6 text-sm text-slate/70">{expert.bio}</p>
      </div>
